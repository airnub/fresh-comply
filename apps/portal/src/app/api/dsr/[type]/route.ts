...
export async function POST(request: NextRequest, { params }: { params: { type: string } }) {
  ...
  if (!isDsrRequestType(type)) {
    return NextResponse.json({ error: "Unsupported request type" } satisfies ErrorBody, { status: 404 });
  }
  ...
  if (!input) {
    return NextResponse.json({ error: "Invalid request payload" } satisfies ErrorBody, { status: 400 });
  }
  ...
  try {
    ...
    if (insertError || !requestRow) {
      ...
      return NextResponse.json({ error: "Unable to record request" } satisfies ErrorBody, { status: 503 });
    }
    ...
    return NextResponse.json({ error: "Unexpected error" } satisfies ErrorBody, { status: 500 });
  }
}
