...
**Schema/Migrations**
- [ ] Ensure/extend **tenant context** on core tables: `organisations(org_id)`, `workflow_runs(org_id)`, `steps(org_id)`, `documents(org_id)`, `audit_log(org_id)`, `admin_actions(org_id)`, `engagements(org_id)`, etc.
- [ ] Create/ensure roles:
...
- **Actor context**: `{ actor_user_id, actor_org_id, on_behalf_of_org_id, parent_org_id, org_id, role }`.
